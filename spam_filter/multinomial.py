...
class MultinomialNaiveBayes:
	...
	def __init__(self, features: np.array, labels: np.array):
		...
		self.class_log_prior[Classification.HAM.value] = math.log(np.sum(labels == Classification.HAM.value))
		self.class_log_prior[Classification.SPAM.value] = math.log(np.sum(labels == Classification.SPAM.value))
		...
		for i in range(features.shape[1]):
			self.feature_log_prob[Classification.HAM.value][i] = math.log(ham_words[i] / ham_sum)
			self.feature_log_prob[Classification.SPAM.value][i] = math.log(spam_words[i] / spam_sum)
	...
	def predict(self, features: np.array):
		...
		for i in range(features.shape[0]):
			...
			for j in range(features.shape[1]):
				ham_prob += (self.feature_log_prob[Classification.HAM.value][j] * features[i][j])
				spam_prob += (self.feature_log_prob[Classification.SPAM.value][j] * features[i][j])
			...
			classes[i] = Classification.HAM.value if ham_prob > spam_prob else Classification.SPAM.value
